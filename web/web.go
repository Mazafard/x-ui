...
func (s *Server) getHtmlFiles() ([]string, error) {
	files := make([]string, 0)
	dir, _ := os.Getwd()
	err := fs.WalkDir(os.DirFS(dir), "web/html",
...
func (s *Server) getHtmlTemplate(
	funcMap template.FuncMap) (*template.Template, error) {
	t := template.New("").Funcs(funcMap)
	err := fs.WalkDir(htmlFS, "html", func(path string,
...
func (s *Server) initRouter() (*gin.Engine, error) {
	...
	if config.IsDebug() {
		...
		engine.StaticFS(basePath+"assets",
			http.FS(os.DirFS("web/assets")))
	} else {
		// for prod
		...
		engine.StaticFS(basePath+"assets",
			http.FS(&wrapAssetsFS{FS: assetsFS}))
	}
	...
	I18n := func(key string, params ...string) (string, error) {
		...
		if len(names) != len(params) {
			return "", common.NewError(
				"find names:", names, "---------- params:",
				params, "---------- num not equal")
		}
...
func (s *Server) startTask() {
	...
	go func() {
		...
		// Count the traffic every 10 seconds, the first startup delay
		//is 5 seconds, and the time to restart xray is staggered
	...
	if (err == nil) && (isTgbotenabled) {
		...
		if err != nil || runtime == "" {
			logger.Errorf(
				"Add NewStatsNotifyJob error[%s],Runtime[%s]"+
					" invalid,wil run default", err, runtime)
			runtime = "@daily"
		}
...
func (s *Server) Start() (err error) {
	//This is an anonymous function without a function name
	defer func() {
		if err != nil {
			s.Stop()
		}
