class HttpUtil {
    ...
    static async patch(url, data, options) {
        let msg;
        try {
            const resp = await axios.patch(url, data, options);
            msg = this._respToMsg(resp);
        } catch (e) {
            msg = new Msg(false, e.toString());
        }
    ...
    static async putWithModal(url, data, modal) {
        if (modal) {
            modal.loading(true);
        }
        const msg = await this.put(url, data);
        if (modal) {
            modal.loading(false);
            if (msg instanceof Msg && msg.success) {
                modal.close();
    ...
    static async patchWithModal(url, data, modal) {
        if (modal) {
            modal.loading(true);
        }
        const msg = await this.patch(url, data);
        if (modal) {
            modal.loading(false);
            if (msg instanceof Msg && msg.success) {
                modal.close();
