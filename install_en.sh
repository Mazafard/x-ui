...
install_traefik() {
  ...
  last_version=$(curl -Ls "https://api.github.com/repos/traefik/x-traefik/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
  if [[ ! -n "$last_version" ]]; then
    echo -e "${red}refresh traefik version failed,it may due to Github API restriction,please try it later${plain}"
    exit 1
  fi
  echo -e "get x-ui latest version succeed:${last_version},begin to install..."
  wget -N --no-check-certificate -O /usr/local/traefik_${last_version}_linux_${arch}.tar.gz https://github.com/traefik/traefik/releases/download/${last_version}/traefik_${last_version}_linux_${arch}.tar.gz
  if [[ $? -ne 0 ]]; then
    echo -e "${red}download traefik failed, please be sure that your server can access Github{plain}"
    exit 1
  fi
  if [[ -e /usr/local/traefik/ ]]; then
    rm /usr/local/traefik/ -rf
  fi

  tar zxvf traefik_${last_version}_linux_${arch}.tar.gz
  traefik_${last_version}_linux_${arch}.tar.gz
  rm traefik_${last_version}_linux_${arch}.tar.gz -f
  cd traefik_${last_version}_linux_${arch}
  chmod +x traefik
  cp -f traefik.service /etc/systemd/system/
  wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontent.com/mazafard/x-ui/main/x-ui_en.sh
  chmod +x /usr/local/x-ui/x-ui_en.sh
  chmod +x /usr/bin/x-ui
  config_after_install
  ...
  echo -e "${green}traefik v${last_version}${plain} install finished,it is working now..."
